#' @title Probabilistic estimation of earthquake recurrence interval using log-normal distribution
#' @description Computes the probability of an earthquake within a specified time "r" and elapsed time "te".
#'
#' @param fit Fit is the fitlnormal object. See ?fitlnormal for details.
...
lnormalcp=function(fit,r,te){
  if (missingArg(fit) == TRUE)
    stop("fit object is missing. The fit object must be fitlnormal object. See ?fitlnormal for details.")
  if (missingArg(r) == TRUE)
    stop("r object is missing.")
  if (missingArg(te) == TRUE)
    stop("te object is missing.")

  GG=cdflnormal(fit$mle,r+te)
  G=cdflnormal(fit$mle,te)
  f=(GG-G)/(1-G)
  return(f)
}
