...
fitiweibull=function(starts,data){
  x=NULL
  ll = function(par, x) {-sum(log(pdfiweibull(par, x))) }
  ...
  KS = ks.test(x = data, y = "cdfiwweibull", par = as.vector(parameters))
  ...
  curve(pdfiweibull(parameters,x),min(data),max(data+1),add=T,col="1",lwd=2,lty=1)
  title("Fitted density")
  probDist <- cdfiwweibull(parameters,data)
