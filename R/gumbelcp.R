#' @title Probabilistic estimation of earthquake recurrence interval using Gumbel distribution
#' @description Computes the probability of an earthquake within a specified time "r" and elapsed time "te".
#'
#' @param fit Fit is the fitgumbel object. See ?fitgumbel for details.
...
gumbelcp=function(fit,r,te){
  if (missingArg(fit) == TRUE)
    stop("fit object is missing. The fit object must be fitgumbel object. See ?fitgumbel for details.")
  if (missingArg(r) == TRUE)
    stop("r object is missing.")
  if (missingArg(te) == TRUE)
    stop("te object is missing.")

  GG=cdfgumbel(fit$mle,r+te)
  G=cdfgumbel(fit$mle,te)
  f=(GG-G)/(1-G)
  return(f)
}
