...
fitexprayleigh=function(starts,data){
  x=NULL
  ll = function(par, x) {-sum(log(pdfer(par, x))) }
  result = optim(par = starts, fn = ll, x = data,method = "Nelder-Mead", hessian = TRUE)
  parameters=result$par
  ylimup=max(pdfer(parameters,data))
  hess=result$hessian
  datasort=sort(data)
  n=length(data)
  p=length(parameters)
  ...
  KS = ks.test(x = data, y = "cdfer", par = as.vector(parameters))
  ...
  curve(pdfer(parameters,x),min(data),max(data+1),add=T,col="1",lwd=2,lty=1)
  title("Fitted density")
  probDist <- cdfer(parameters,data)
