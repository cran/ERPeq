...
fitggamma=function(starts,data){
  x=NULL
  ll = function(par, x) {-sum(log(pdfggamma(par, x))) }
  ...
  KS = ks.test(x = data, y = "cdfggamma", par = as.vector(parameters))
  ...
  curve(pdfggamma(parameters,x),min(data),max(data+1),add=T,col="1",lwd=2,lty=1)
  title("Fitted density")
  probDist <- cdfggamma(parameters,data)
