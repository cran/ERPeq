...
fitlnormal=function(starts,data){
  x=NULL
  ll = function(par, x) {-sum(log(pdflnormal(par, x))) }
  ...
  KS = ks.test(x = data, y = "cdflnormal", par = as.vector(parameters))
  ...
  curve(pdflnormal(parameters,x),min(data),max(data+1),add=T,col="1",lwd=2,lty=1)
  title("Fitted density")
  probDist <- cdflnormal(parameters,data)
