...
fitgamma=function(starts,data){
  x=NULL
  ll = function(par, x) {-sum(log(pdfgamma(par, x))) }
  ...
  KS = ks.test(x = data, y = "cdfgamma", par = as.vector(parameters))
  ...
  curve(pdfgamma(parameters,x),min(data),max(data+1),add=T,col="1",lwd=2,lty=1)
  title("Fitted density")
  probDist <- cdfgamma(parameters,data)
