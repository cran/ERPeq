...
fitweibull=function(starts,data){
  x=NULL
  ll = function(par, x) {-sum(log(pdfweibull(par, x))) }
  ...
  KS = ks.test(x = data, y = "cdfweibull", par = as.vector(parameters))
  ...
  curve(pdfweibull(parameters,x),min(data),max(data+1),add=T,col="1",lwd=2,lty=1)
  title("Fitted density")
  probDist <- cdfweibull(parameters,data)
